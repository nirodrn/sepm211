...
const AddUser = () => {
  ...
  const loadShowrooms = async () => {
    try {
      const activeShowrooms = await directShowroomService.getActiveShowrooms();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const userData = {
        ...
        showroom_id: formData.showroomId || null,
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Showroom Selection */}
            {(formData.role === 'DirectShowroomManager' || formData.role === 'DSStaff') && (
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Assigned Showroom *
                </label>
                ...
                >
                  ...
                  {showrooms.map(showroom => (
                    <option key={showroom.id} value={showroom.id}>
                      {showroom.name} ({showroom.code}) - {showroom.city}
                    </option>
                  ))}
                </select>
                {showrooms.length === 0 && (
                  <p className="text-sm text-yellow-600 mt-1">
                    No showrooms found. Please create a showroom first.
                  </p>
                )}
                <p className="text-sm text-gray-500 mt-1">
                  Required: Select the showroom this user will be assigned to
                </p>
              </div>
            )}
