...
export const fgDispatchService = {
  ...
  async dispatchSalesRequest(requestId, dispatchData) {
    try {
      ...
      for (const [itemId, dispatchItem] of Object.entries(items)) {
        if (dispatchItem.batches && Array.isArray(dispatchItem.batches)) {
          for (const batch of dispatchItem.batches) {
            if (batch.inventoryType === 'bulk') {
              const invKey = `${approvedItems[itemId].productId || itemId}_${batch.batchNumber}`;
              const currentInv = await getData(`finishedGoodsInventory/${invKey}`);
              if (currentInv) {
                const newQty = Math.max(0, (currentInv.quantity || 0) - batch.quantity);
              ...
              const productId = approvedItems[itemId].productId || itemId;
              const invData = await getData(`finishedGoodsPackagedInventory`);
              if (invData) {
                for (const [key, inv] of Object.entries(invData)) {
                  if (inv.batchNumber === batch.batchNumber && inv.productId === productId) {
                    const newUnits = Math.max(0, (inv.unitsInStock || 0) - batch.quantity);
                    await updateData(`finishedGoodsPackagedInventory/${key}`, {
