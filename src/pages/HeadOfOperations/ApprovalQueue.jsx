...
import { directRepresentativeService } from '../../services/directRepresentativeService';
...
const ApprovalQueue = () => {
  ...
  const handleApproveRequest = async (request) => {
    try {
      console.log('=== APPROVAL QUEUE - handleApproveRequest ===');
      console.log('Request Type:', request.requestType);
      console.log('Request ID:', request.id);
      console.log('Request items:', JSON.stringify(request.items, null, 2));
      ...
      // Use the appropriate service based on request type
      if (request.requestedByRole === 'DirectRepresentative' || request.requestType === 'direct_representative') {
        console.log('>>> Calling directRepresentativeService.approveRequest with ID:', request.id);
        const result = await directRepresentativeService.approveRequest(request.id, approverInfo);
        console.log('>>> Direct Representative approval result:', result);
        alert(`Direct Representative request approved! Check console for details. History ID: ${result.historyId}`);
      } else {
        // For Distributor and DirectShop requests
        console.log('>>> Calling distributorRequestService.approveRequest');
      ...
      const requestTypeLabel = request.requestedByRole === 'DirectRepresentative' ? 'Direct Representative' :
                               request.requestedByRole === 'DirectShop' ? 'Direct Shop' : 'Distributor';
      setError(`Failed to approve ${requestTypeLabel} request: ${err.message}`);
      console.error('Error approving request:', err);
      console.error('Request data that caused error:', request);
    }
  };
  ...
  const handleRejectRequest = async (request) => {
    try {
      // Use the appropriate service based on request type
      if (request.requestedByRole === 'DirectRepresentative' || request.requestType === 'direct_representative') {
        await directRepresentativeService.rejectRequest(request.id, request);
      } else {
      ...
      const requestTypeLabel = request.requestedByRole === 'DirectRepresentative' ? 'Direct Representative' :
                               request.requestedByRole === 'DirectShop' ? 'Direct Shop' : 'Distributor';
      setError(`Failed to reject ${requestTypeLabel} request: ${err.message}`);
      console.error('Error rejecting request:', err);
    }
  };
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                        ...
                        <div className="md:col-span-2">
                          ...
                          <div className="mt-1 space-y-1">
                            {request.items && Object.keys(request.items).length > 0 ? (
                              Object.entries(request.items).map(([key, item]) => (
                                <div key={key} className="flex items-center justify-between bg-gray-50 px-3 py-2 rounded">
                                  <span className="font-medium">{item.name || key}</span>
                                  <span className="text-sm font-semibold text-blue-600">Qty: {item.qty || item.quantity || 0}</span>
                                </div>
                              ))
                              ...
                              <div className="flex items-center justify-between bg-gray-50 px-3 py-2 rounded">
                                ...
                                <span className="text-sm font-semibold text-blue-600">Qty: {request.quantity}</span>
                              </div>
                            ) : (
                              <div className="bg-gray-50 px-3 py-2 rounded text-gray-500">
                                No items specified
                              </div>
                            )}
                          </div>
