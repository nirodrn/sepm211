...
import { Send, Search, Filter, Eye, Store, User, Truck as TruckIcon, Package, Calendar, MapPin, BarChart3, Download, Clock, CheckCircle, AlertTriangle, Warehouse, X } from 'lucide-react';
...
const ExternalDispatches = () => {
  ...
  const handleDispatchRequest = async (request) => {
    try {
      // Load inventory for this request
      const [bulkInv, packagedInv] = await Promise.all([
        fgStoreService.getInventory(),
        fgStoreService.getPackagedInventory()
      ]);

      setAvailableInventory([...bulkInv, ...packagedInv]);

      // Prepare items with editable prices
      let itemsWithPrices = [];

      if (Array.isArray(request.items)) {
        itemsWithPrices = request.items.map(item => ({
          ...item,
          unitPrice: item.unitPrice || 100,
          discountPercent: 0,
          finalPrice: item.unitPrice || 100,
          selectedBatches: [],
        ...
        itemsWithPrices = Object.entries(request.items || {}).map(([id, item]) => ({
          productName: item.name,
          quantity: item.qty,
          unitPrice: 100,
          discountPercent: 0,
  ...
  const confirmDispatch = async () => {
    ...
    for (const item of dispatchData.items) {
      if (!item.selectedBatches || item.selectedBatches.length === 0) {
        setError(`Please select batches for ${item.productName}`);
        return;
      }
      const totalSelected = item.selectedBatches.reduce((sum, b) => sum + b.quantityToDispatch, 0);
      if (totalSelected < item.quantity) {
        setError(`Insufficient quantity selected for ${item.productName}. Need ${item.quantity}, selected ${totalSelected}`);
        return;
      }
    }
    ...
    try {
      ...
      // Calculate total value from dispatch items with final prices
      const totalValue = dispatchData.items.reduce((sum, item) =>
        sum + (item.quantity * item.finalPrice), 0
      );

      // Decrease stock from selected batches
      for (const item of dispatchData.items) {
        for (const batch of item.selectedBatches) {
          // Use the id field which contains the Firebase key
          const invKey = batch.id;

          if (batch.type === 'bulk') {
            const currentInv = await getData(`finishedGoodsInventory/${invKey}`);

            if (currentInv) {
              const newQty = Math.max(0, (currentInv.quantity || 0) - batch.quantityToDispatch);
              await updateData(`finishedGoodsInventory/${invKey}`, {
                quantity: newQty,
            ...
            const currentInv = await getData(`finishedGoodsPackagedInventory/${invKey}`);

            if (currentInv) {
              const newUnits = Math.max(0, (currentInv.unitsInStock || 0) - batch.quantityToDispatch);
              await updateData(`finishedGoodsPackagedInventory/${invKey}`, {
          ...
          await fgStoreService.recordInventoryMovement({
            ...
            type: 'out',
            quantity: batch.quantityToDispatch,
            reason: `Dispatched to ${selectedRequest.requestedByName || selectedRequest.requesterName}`,
      ...
      await updateData(requestPath, {
        ...
        dispatchItems: dispatchData.items,
  ...
  const handleBatchSelect = (itemIndex, batch, quantityToDispatch) => {
    const newItems = [...dispatchData.items];
    const item = newItems[itemIndex];

    // Initialize selectedBatches if not exists
    if (!item.selectedBatches) {
      item.selectedBatches = [];
    }

    // Check if batch already selected
    const existingIndex = item.selectedBatches.findIndex(b =>
      b.batchNumber === batch.batchNumber && b.location === batch.location
    );

    if (existingIndex >= 0) {
      // Update quantity
      if (quantityToDispatch > 0) {
        item.selectedBatches[existingIndex].quantityToDispatch = quantityToDispatch;
      } else {
        // Remove if quantity is 0
        item.selectedBatches.splice(existingIndex, 1);
      }
    } else if (quantityToDispatch > 0) {
      // Determine batch type
      const batchType = batch.type || (batch.unit ? 'bulk' : 'packaged');

      // Add new batch selection
      item.selectedBatches.push({
        ...batch,
        type: batchType,
        quantityToDispatch
      });
  ...
  const getTotalSelectedForItem = (itemIndex) => {
    const item = dispatchData.items[itemIndex];
    return item.selectedBatches?.reduce((sum, b) => sum + b.quantityToDispatch, 0) || 0;
  };

  const getSelectedQuantityForBatch = (itemIndex, batch) => {
    const item = dispatchData.items[itemIndex];
    const selected = item.selectedBatches?.find(b =>
      b.batchNumber === batch.batchNumber && b.location === batch.location
    );
  ...
  const getAvailableBatchesForItem = (item) => {
    // Filter inventory for this product - more lenient matching
    const productInventory = availableInventory.filter(inv => {
      if (!inv.productName || !item.productName) return false;

      const invName = inv.productName.toLowerCase().trim();
      const itemName = item.productName.toLowerCase().trim();
      const nameMatch = invName === itemName || invName.includes(itemName) || itemName.includes(invName);

      // Check if has stock
      const bulkStock = inv.quantity && Number(inv.quantity) > 0;
      const unitStock = inv.unitsInStock && Number(inv.unitsInStock) > 0;
      const hasStock = bulkStock || unitStock;
                ...
                <div className="space-y-4">
                  {dispatchData.items.map((item, index) => {
                    const handlePriceChange = (value) => {
                      ...
                      newItems[index].finalPrice = newItems[index].unitPrice * (1 - discount / 100);
                      setDispatchData(prev => ({ ...prev, items: newItems }));
                    };

                    const handleDiscountChange = (value) => {
                      const newItems = [...dispatchData.items];
                      newItems[index].discountPercent = parseFloat(value) || 0;
                      newItems[index].finalPrice = newItems[index].unitPrice * (1 - newItems[index].discountPercent / 100);
                      setDispatchData(prev => ({ ...prev, items: newItems }));
                    };

                    const totalSelected = getTotalSelectedForItem(index);
                    const hasEnoughSelected = totalSelected >= item.quantity;
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center justify-between border-b border-gray-200 pb-3">
                            <div className="flex-1">
                              <p className="font-semibold text-lg text-gray-900">{item.productName}</p>
                              <p className="text-sm text-gray-600">Required Quantity: {item.quantity} units</p>
                              <div className={`text-sm font-medium mt-1 ${hasEnoughSelected ? 'text-green-600' : 'text-orange-600'}`}>
                                Selected: {totalSelected} / {item.quantity} {hasEnoughSelected ? '✓' : ''}
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="bg-blue-50 border border-blue-200 rounded p-2">
                            <div className="flex items-center justify-between text-sm">
                              <span className="text-blue-700">Final Price per unit:</span>
                              <span className="font-bold text-blue-900">LKR {item.finalPrice.toFixed(2)}</span>
                            </div>
                          </div>

                          <div className="border-t border-gray-200 pt-3">
                            <p className="text-sm font-semibold text-gray-900 mb-2">Select Batches & Locations:</p>
                            {availableBatches.length === 0 ? (
                              <div className="text-center py-4 bg-red-50 rounded-lg">
                                <Package className="mx-auto h-8 w-8 text-red-400" />
                                <p className="mt-2 text-sm text-red-600">No inventory available for this product</p>
                              </div>
                            ) : (
                              <div className="space-y-2 max-h-60 overflow-y-auto">
                                {availableBatches.map((batch, batchIdx) => {
                                  const isBulk = batch.quantity !== undefined && batch.unit;
                                  const isPackaged = batch.unitsInStock !== undefined;
                                  const maxAvailable = isBulk ? (batch.quantity || 0) : (batch.unitsInStock || 0);
                                  const batchType = isBulk ? 'bulk' : 'packaged';
                                  const selectedQty = getSelectedQuantityForBatch(index, batch);

                                  return (
                                    <div key={batchIdx} className="border border-gray-200 rounded-lg p-3 bg-gray-50">
                                      <div className="flex items-start justify-between">
                                        <div className="flex-1">
                                          <div className="flex items-center space-x-2 mb-1">
                                            <Warehouse className="h-4 w-4 text-gray-600" />
                                            <span className="font-medium text-sm text-gray-900">Batch: {batch.batchNumber}</span>
                                            <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${
                                              isBulk ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
                                            }`}>
                                              {isBulk ? 'Bulk' : 'Packaged'}
                                            </span>
                                          </div>
                                          <div className="text-xs text-gray-600 space-y-0.5">
                                            <p><MapPin className="h-3 w-3 inline mr-1" />Location: <span className="font-medium">{batch.location}</span></p>
                                            <p>Available: <span className="font-medium">{maxAvailable} {isBulk ? batch.unit : 'units'}</span></p>
                                            {batch.qualityGrade && <p>Quality: Grade {batch.qualityGrade}</p>}
                                            {batch.variantName && <p>Variant: {batch.variantName}</p>}
                                          </div>
                                        </div>
                                        <div className="ml-3 w-24">
                                          ...
                                          <input
                                            ...
                                            onChange={(e) => {
                                              ...
                                              handleBatchSelect(index, { ...batch, type: batchType }, qty);
                                            }}
                          ...
                          <div className="flex items-center justify-between pt-2 border-t border-gray-200">
                            ...
                            <div className="text-right">
                              <p className="font-bold text-gray-900">
                                LKR {(item.quantity * item.finalPrice).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                              </p>
                              {item.discountPercent > 0 && (
                                <p className="text-xs text-gray-500 line-through">
                                  LKR {(item.quantity * item.unitPrice).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </p>
                              )}
                            </div>
                          </div>
                  ...
                  <div className="bg-blue-100 border border-blue-300 rounded-lg p-3 mt-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-lg font-bold text-blue-900">
                        LKR {dispatchData.items.reduce((sum, item) =>
                          sum + (item.quantity * item.finalPrice), 0
                        ).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                    </div>
                    {dispatchData.items.some(item => item.discountPercent > 0) && (
                      <div className="flex items-center justify-between text-sm mt-1">
                        ...
                        <span className="font-medium text-green-600">
                          - LKR {dispatchData.items.reduce((sum, item) =>
                            sum + (item.quantity * (item.unitPrice - item.finalPrice)), 0
                          ).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </span>
                      </div>
                    )}
